...
- `updated_final_owners.md` — tab-separated lookup table of ultimate economic owners for all 56 Spain ECT cases. **Researched by Gemini (Google AI) in June 2025** using public corporate data, fund prospectuses, and ICSID records; reviewed but not independently audited. Used to populate the `ULT_OWNER` map in `index.html` (inner ring of the origin doughnut chart). Cases with diversified or unknown ownership are excluded from `ULT_OWNER`.
- `unknown_final_owner.md` — list of Spain ECT cases where ultimate ownership remains unresolved after the Gemini research pass. Maintained alongside `updated_final_owners.md`.
